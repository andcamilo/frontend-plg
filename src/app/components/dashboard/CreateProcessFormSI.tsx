...
const CreateProcessFormSI = ({
  ...
  // Check if all required fields are filled but no solicitudes available
  const allFieldsFilled = form.email && form.tipoTramite && form.tipoSolicitud && emailExists;
  const showNoSolicitudesError = allFieldsFilled && solicitudes.length === 0;
  ...
  return (
    <form onSubmit={handleFormSubmitSI} className="flex flex-col gap-4">
      ...
      {showNoSolicitudesError && (
        <div className="text-red-500 font-bold text-center py-2 bg-red-100 bg-opacity-10 rounded-lg">
          No hay solicitudes disponibles para este usuario y tipo de trámite seleccionado.
        </div>
      )}
      ...
      <button
        type="submit"
        disabled={isSubmitting || showNoSolicitudesError}
        className={`bg-profile text-white font-bold py-3 px-6 rounded-lg transition-colors mt-2 flex items-center justify-center ${
          isSubmitting || showNoSolicitudesError ? 'opacity-50 cursor-not-allowed' : 'hover:bg-pink-700'
        }`}
      >
        {isSubmitting ? (
          <>
